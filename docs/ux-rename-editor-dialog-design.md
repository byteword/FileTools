...
- Internet dictionary or AI-assisted automatic correction. The supported
  extension path is now a review-only correction plugin API; see
  `docs/rename-correction-plugin-design.md`.
...
- Plugin candidate providers: optional offline or online providers loaded
  through the correction plugin API, with a FileTools language setting and
  plugin-specific settings generated by the host.
