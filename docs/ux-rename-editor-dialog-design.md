...
Updated on 2026-06-24 for `1.4.6.1`: recommendation chips now tokenize only the
current original name, and automatic correction restores obfuscated Hangul
candidates from the current input before falling back to the stored automatic
name.
Updated on 2026-06-24 for `1.4.6.2`: the simple review dialog now initializes
...
- The first generated rename row is selected safely after the list items are
  available, including the one-file flow where the list column is hidden.
- `자동교정` sits next to `고급` in the simple confirmation. It applies the
  same correction fallback as the advanced editor to the selected new name;
  restoring the original name remains an advanced-editor-only action.
...
- Recommended text chips are generated by tokenizing the current original name;
  referenced source file names are not added wholesale.
- The automatic correction action applies obfuscated Hangul restoration
  candidates to the current input before falling back to the stored automatic
  name.
