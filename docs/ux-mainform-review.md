...
The target grid shows per-target action counts, and the bottom plan grid can now switch between all plan, selected targets, and warnings. Shared operations show explicit group headers plus grouped input rows for archive merge and duplicate-delete steps.
Connector-style row markers are now rendered in the action column for grouped input rows, so one logical operation is visually grouped across multiple source rows.
