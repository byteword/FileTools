...
- Obfuscated Hangul candidate generation: inline candidate scoring word and protected English word editors backed by `rename-candidate-profile.json`. The built-in replacement table remains internal but includes short review candidates such as `ㅇr -> 아` and `ㅎH -> 해`, and mixed precomposed-syllable fragments such as `혀ㄴ주ㅇ구l호rㄴ로ㄱ -> 현중귀환록`.
